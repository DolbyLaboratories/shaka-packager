...
class AC4Parser {
 public:
  ...
  virtual bool Parse(const uint8_t* data,
                     size_t data_size);
  const int GetAc4TocSize() { return toc_size; };
 ...
 private:
  ...
  int AC4Parser::ParseChannelMode(BitReader* reader,
                                  int presentation_version);
  ...
  bool ParseAc4SubstreamInfoObj(BitReader* reader,
                                ...
                                int frame_rate_factor,
                                int b_substreams_present);
  const int GetPresentationIdx(int substream_group_index);
  const int GetPresentationVersion(int substream_group_index);
